# Предварительный анализ задачи определения мошеннических финансовых операций

**Домашнее задание №1**  
**Курс MLOps**  
**Образовательная платформа «Otus»**

## Цель работы

В данном домашнем задании вы потренируетесь формализовывать поставленные бизнесом задачи анализа данных, определять цели и способы их достижения, декомпозировать систему анализа данных и визуализировать задачи с использованием канбан-доски в GitHub Projects.

## Описание проекта

> Вас пригласили на работу в качестве специалиста по данным в коммерческую компанию, занимающуюся предоставлением банковских услуг населению. Одной из услуг компании, пользующейся большой популярностью у населения, является проведение онлайн-платежей с банковских счетов граждан.
>
> К сожалению, в последнее время участились случаи мошенничества при проведении таких операций. Руководство компании встревожено такой ситуацией. На ближайшем совещании было принято решение внедрить в IT-инфраструктуру компании модуль на базе машинного обучения, способный в реальном времени оценивать, является ли проводимая транзакция мошеннической.
>
> Это, в свою очередь, позволило бы системе онлайн-платежей отклонить подозрительную операцию. Вам предлагают принять участие в проектировании и построении такой системы.

## Требования заказчика

В ходе детального обсуждения проекта с представителями заказчика выяснились следующие моменты:

> ### Бюджетные ограничения
>
> Руководство готово выделить на реализацию данного проекта не более **10 млн рублей** (не считая зарплат специалистам). Через три месяца оно хотело бы увидеть первые результаты, что позволило бы понять, стоит ли дальше развивать проект. В случае положительного решения, проект нужно завершить за полгода.
>
> ### Требования к качеству системы
>
> Из открытых источников известно, что у ближайших конкурентов на каждую сотню проводимых их клиентами транзакций фиксируется не более двух мошеннических, приводящих к потере денежных средств. При этом общий ущерб клиентов за месяц не превышает **500 тыс. рублей**.
>
> Разрабатываемая система должна выдавать результаты не хуже, иначе компания станет неконкурентоспособной.
>
> ### Производительность системы
>
> В среднем, компания обрабатывает около **50 транзакций в секунду**, однако, перед праздниками это число может достигать **400**.
>
> ### Требования к ложным срабатываниям
>
> Если система определит корректную транзакцию как мошенническую, эта транзакция будет отклонена, а пользователь будет недоволен. Опыт бизнес-аналитиков подсказывает, что если доля таких транзакций превысит **5%**, то начнется отток клиентов из компании.
>
> ### Инфраструктурные ограничения
>
> Представители компании не готовы размещать разрабатываемый модуль на собственных вычислительных ресурсах.
> Рекомендуется воспользоваться облачными ресурсами.
>
> ### Формат данных
>
> При проведении транзакции системой вся информация о ней сохраняется в виде CSV-файлов, каждая строка которых соответствует одной транзакции. Информация о транзакциях некоторого фиксированного периода помещается в отдельный файл.
>
> ### Конфиденциальность
>
> Файлы с данными содержат всю информацию о транзакциях, включая данные клиента, который ее проводил. Данная информация является конфиденциальной, ее утечка недопустима.
>
## Задания

Вам предлагается на основе представленной информации:

### Обязательные задания

1. **Сформулировать цели** проектируемой антифрод-системы в соответствии с требованиями заказчика.

2. **Аргументированно выбрать метрику** машинного обучения, оптимизация которой, на ваш взгляд, позволит достичь поставленных целей.

3. **Проанализировать особенности проекта** с использованием, например, MISSION Canvas. Это позволит выделить специфику проекта и формализовать требования к нему, подобрать инструментарий и критерии оценки.

### Дополнительные задания

4. **Попытаться декомпозировать** планируемую систему, определить ее основные функциональные части.

5. **Определить задачи**, решение которых необходимо для достижения поставленных целей с учетом проведенного анализа. Задачи рекомендуется формулировать по принципу S.M.A.R.T. На текущий момент, пока не конкретизированы детали антифрод-системы, они могут быть представлены в достаточно общем виде. Но они позволят сформировать некоторый roadmap по реализации проекта.

6. **Создать GitHub-репозиторий** для проекта и разместить в нем сформулированные задачи (~5 задач) на Kanban-доске в GitHub Projects со статусом «Новые». В дальнейшем, при реализации проекта, эти задачи будут конкретизироваться, дробиться, видоизменяться и выполняться. Их статус тоже будет изменяться, что позволит вам наглядно оценивать степень завершенности проекта.

## Критерии оценки

Для получения положительной оценки за работу необходимо выполнить **минимум первые три** вышеприведенные задания.

---

## 1. Цели проектируемой антифрод‑системы

На основе требований заказчика можно сформулировать следующие цели:

### 1. Финансовая эффективность: 
Уровень ущерба от мошенничества не должен превышать 500 тыс. рублей в месяц. Это ключевой бизнес-показатель (KPI)

### 2. Качество детекции (False Negative Rate):
Обеспечить долю пропущенных мошеннических транзакций (False Negative Rate) не более 2% от общего числа мошеннических транзакций 
(аналогично ближайшим конкурентам).

### 3. Минимизация ложных срабатываний (False Positive Rate): 
Обеспечить долю ложных срабатываний (False Positive Rate) не более 5%, чтобы избежать оттока клиентов.

### 4. Обеспечение производительности:
Система должна обрабатывать транзакции в реальном времени (латентность ≤ 100 ms при средней нагрузке 50 tps, с поддержкой до 400 tps при пиковых нагрузках).

### 5. Соблюдение требований безопасности и конфиденциальности:
Все данные должны обрабатываться в защищённой облачной инфраструктуре с соблюдением регуляторных норм (например, ФЗ-152 в РФ, GDPR при необходимости).

### 6. Бюджет и сроки: 
Уложиться в бюджет 10 млн руб. (на облачную инфраструктуру и сторонние сервисы) и представить первые результаты через 3 месяца (MVP), а финальное решение — через 6 месяцев.


## 2. Метрика машинного обучения

У нас есть два критически важных, но конфликтующих требования: поймать как можно больше мошенников (Recall) и при этом не мешать честным клиентам (Precision).

Рекомендуемая метрика для оптимизации: `F-Beta Score`

**Аргументация:**

`Precision (Точность) = True Positive / (True Positive + False Positive)`. Показывает, какая доля транзакций, которые мы назвали мошенническими, и правда ими является. Высокая Precision критична для удержания требования по `FPR < 5%`.

`Recall (Полнота) = True Positive / (True Positive + False Negative)`. Показывает, какую долю реальных мошеннических транзакций мы смогли обнаружить. Высокий Recall критичен для финансовой цели (ущерб < 500 тыс. руб.).

`F1-Score` — это среднее гармоническое между Precision и Recall. Однако в нашем случае Recall важнее, так как стоимость пропуска мошенничества (False Negative) — это прямые финансовые потери клиента и компании, а стоимость ложного срабатывания (False Positive) — это недовольство клиента, которое, хотя и критично, но все же менее катастрофично, чем потеря денег.

Поэтому предлагается `F-beta Score`, где `beta > 1`, чтобы придать больше веса Recall.

Предлагаемая метрика: `F2-Score`.

Формула: `F2 = (1 + 2^2) * (Precision * Recall) / (4 * Precision + Recall)`

Коэффициент beta = 2 означает, что для нас Recall в 2 раза важнее, чем Precision. Это напрямую следует из бизнес-требований: лучше отклонить на 1-2% больше легитимных транзакций (но остаться в рамках 5%), чем пропустить мошенников и не выполнить финансовый KPI.

## 3. Анализ проекта с использованием MISSION Canvas

![](./business-model-canvas-otus.png)

## 4. Декомпозиция системы (основные функциональные блоки)

### Ingestion & Secure Storage

- Безопасный загрузчик CSV в защищённое облачное хранилище (шифрование at‑rest, IAM, журналирование).

- Потоковый приём транзакций (Kafka / PubSub / Kinesis) для real‑time скоринга.

### Data Processing & Feature Store

- Batch ETL для исторических данных (pandas/Spark).

- Real‑time feature computation (stateful streaming или feature store с materialized views).

- -Feature engineering (бизнес‑фичи, поведенческие фичи, агрегаты по счёту и времени).

### Model Training & Experimentation

- MLOps: код, эксперименты, репликация (MLflow).

- Baseline модели (лог. регрессия / xgboost / lightgbm) и продвинутые (GBoost / NN) при необходимости.

- Кросс‑валидация, временные валидации, таргетирование по времени.

### Real‑time Scoring Service

- Low‑latency API (gRPC/REST), контейнеризация (k8s / serverless), автоскейлинг.

- Thresholding & decision logic (score → блок/флаг/soft‑review).

### Action & Workflow Integration

- Интеграция с платёжной системой: блокировка транзакции, soft‑decline или превью для оператора.

- Интерфейс для ручной проверки и разметки (case management).

### Monitoring & Feedback Loop

- Метрики модели: метрики производительности, метрики сервиса, ML-метрики;
  сбор c prometheus и визуализация в Grafana.

- ML‑drift detection, alerting, periodic retraining.

### Privacy, Compliance & Security

- Обезличивание персональных данных, реализация RBAC, аудит, соответствие требованиям законодательства.

### CI/CD & Infra Automation

- Terraform/Clouds, СI/CD, A/B тестирование.

## 5. SMART‑задачи

### Задача 1. Безопасный сбор и хранение данных (Обезличивание, облако)

- **Specific:** Разработать безопасный ETL-пайплайн для загрузки CSV-файлов с транзакциями в облачное хранилище с автоматическим обезличиванием PII (hash/email/phone masking) и шифрованием на уровне S3/KMS.
- **Measurable:** Пайплайн должен корректно обработать 100 % тестовых файлов без ошибок и удалить/маскировать все поля PII.
- **Achievable:** Используются стандартные инструменты (Python, Pandas, AWS S3, Terraform).
- **Relevant:** Обеспечивает соответствие требованиям конфиденциальности и создаёт базу для обучения модели.
- **Time-bound:** Завершить за 2 недели от старта проекта.

### Задача 2. Анализ данных (EDA) и оценка финансовых потерь

- **Specific:** Провести EDA обезличенных транзакций, оценить долю мошеннических операций и рассчитать ориентировочные месячные потери компании.
- **Measurable:** Подготовить отчёт с минимум 5 визуализациями, summary-таблицей и базовой оценкой метрик (fraud rate, median loss).
- **Achievable:** Используются Jupyter Notebook, Pandas, Matplotlib.
- **Relevant:** Помогает понять бизнес-масштаб проблемы и требования к модели.
- **Time-bound:** Завершить в течение 1 недели после задачи 1.

### Задача 3. Базовая ML-модель и оценка качества

- **Specific:** Построить baseline-модель LightGBM для классификации транзакций как мошеннических/немошеннических.
- **Measurable:** Достичь Recall ≥ 0.9 при FPR ≤ 0.05 на валидации; вычислить ROC-AUC, Precision, Recall.
- **Achievable:** Используются sklearn, LightGBM, joblib; обучение на облачных данных.
- **Relevant:** Даёт первую количественную оценку эффективности антифрод-модуля.
- **Time-bound:** Завершить за 3 недели от начала проекта.

### Задача 4. Прототип API-сервиса скоринга в реальном времени

- **Specific:** Реализовать REST API на FastAPI, способное обрабатывать до 400 запросов/секунду для оценки риска транзакции в реальном времени.
- **Measurable:** Нагрузочный тест (Locust) показывает среднюю задержку ≤ 200 мс и 99-й перцентиль ≤ 400 мс.
- **Achievable:** Разворачивается в облаке (AWS Lambda / EC2) с использованием обученной модели.
- **Relevant:** Прямое требование бизнеса к производительности антифрод-системы.
- **Time-bound:** Завершить через 2 недели после задачи 3.

### Задача 5. Мониторинг и автоматическое переобучение

- **Specific:** Настроить MLflow + Airflow-пайплайн, отслеживающий метрики модели и автоматически запускающий переобучение при деградации Recall > 10 %.
- **Measurable:** MLflow хранит не менее 5 экспериментов; Airflow DAG выполняется по расписанию раз в неделю без ошибок.
- **Achievable:** Используются существующие облачные ресурсы и уже обученная модель.
- **Relevant:** Обеспечивает устойчивость и адаптивность антифрод-системы.
- **Time-bound:** Завершить к концу 6-й недели проекта.

## 6. **Создать GitHub-репозиторий** для проекта и разместить в нем сформулированные задачи (~5 задач) на Kanban-доске в GitHub Projects со статусом «Новые».

https://github.com/users/Deron-D/projects/3